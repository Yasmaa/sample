...
func NewJWTMaker(secretKey string) (*JwtMaker, error) {
	if len(secretKey) < minLengthSecretKey {
		return nil, fmt.Errorf("minimum length of secret key is %v", minLengthSecretKey)
	}
	return &JwtMaker{
		secretKet: secretKey,
	}, nil
...
func (j *JwtMaker) CreateToken(id uint,email string, duration time.Duration) (string, error) {
